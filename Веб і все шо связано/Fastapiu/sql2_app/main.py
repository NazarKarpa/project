...
@app.get("/users/", response_model=List[shemas.User])
...
@app.get("/items/", response_model=List[shemas.Item])
def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    items = crud.get_items(db, skip=skip, limit=limit)
    return items

@app.patch("/update_email/{user_id}", response_model= shemas.UserBase)
def update_user_email(user_id: int, new_email: shemas.UserBase, db: Session = Depends(get_db)):
    return crud.update_user_email(db=db, user_id=user_id, update_data=new_email)


@app.patch("/delete_user/{user_id}")
def delete_user(user_id: int, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    crud.delete_user(db=db, user=user)
    return 'User was deleted succesessfuly'
